...
if (isset($_POST['add-book']) && $_POST['add-book']) {
  ...
  if ($check === false) {
    ...
    $sql = "INSERT INTO book (MaSach,TenSach,TacGia,NhaXB,DonGia,MaTheLoai,HinhAnh) VALUES (?, ?, ?, ?, ?, ?, ?)";
    try {
      $statement = $pdo->prepare($sql);
      $statement->execute([$IdBook, $NameBook, $IdAuthor, $nxb,  $PriceBook, $IdDM, $img]);
...
elseif (isset($_GET['query']) &&  $_GET['query'] == 'delete') {
  ...
  if ($_FILES['img-book'] != '') {
    ...
    $id_edit = $_GET['id'];
    $sqlu = "UPDATE book SET MaSach = ?,TenSach = ?,TacGia = ?,NhaXB = ?,DonGia = ?,MaTheLoai = ?, HinhAnh = ? WHERE MaSach LIKE '%$id_edit%'";
    $tmpImg = $_FILES["img-book"]["tmp_name"];
    $uploads_dir = 'uploads/';
    $img = $_FILES["img-book"]["name"];
    $uploads_file = $uploads_dir . basename($_FILES["img-book"]["name"]);
